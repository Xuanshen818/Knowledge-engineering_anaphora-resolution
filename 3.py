...
class CoreferenceModel(nn.Module):
    def __init__(self, input_dim, hidden_dim1, hidden_dim2, output_dim):
        ...
        self.embedding = nn.Embedding(input_dim, hidden_dim1)
        self.fc1 = nn.Linear(hidden_dim1 * 4, hidden_dim2)
...
if len(dataset) == 0:
    ...
    # 实例化模型
    input_dim = len(read_vocabulary(noun_vocab_file))  # 输入维度为名词词汇表的大小
    hidden_dim1 = 60
    hidden_dim2 = 128
    output_dim = 1
    model = CoreferenceModel(input_dim, hidden_dim1, hidden_dim2, output_dim)

    # 定义损失函数和优化器
    criterion = nn.BCEWithLogitsLoss()  # 修改损失函数
    optimizer = optim.Adam(model.parameters(), lr=0.0001)  # 修正学习率
        ...
        for epoch in range(num_epochs):
            ...
            for i, data in enumerate(dataloader):
                ...
                if (i+1) % 100 == 0:
                    ...
                    print(f'  Batch [{i+1}/{len(dataloader)}], Loss: {epoch_loss:.4f}, Accuracy: {epoch_accuracy:.2f}%')
