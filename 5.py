...
for i in range(len(features)):
    for j in range(len(features[i])):
        if features[i][j].shape != features[0][0].shape:
            features[i][j] = np.zeros_like(features[0][0])
...
padded_labels = []
for label in labels:
    padded_label = np.zeros(max_label_length)
...
# 损失函数和优化器
class_weight = torch.tensor([1, (len(train_labels) - sum(sum(train_labels))) / sum(sum(train_labels))], dtype=torch.float32)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            total += labels.size(0)
            correct += (predicted == labels.view(-1, 1)).sum().item()  # 调整预测结果的形状
