...
def train_model(model, criterion, optimizer, dataloader, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        for data in dataloader:
            ...
            running_loss += loss.item() * len(pronoun_number)
            predicted = torch.round(outputs)
            total += len(pronoun_number)
            correct += (predicted == pronoun_number.view(-1)).sum().item()  # 调整正确预测的计算方式

        epoch_loss = running_loss / total
        loss_values.append(epoch_loss)
        print('Epoch [{}/{}], Loss: {:.4f}, Accuracy: {:.2f}%'.format(epoch + 1, num_epochs, epoch_loss,
                                                                      100 * correct / total))
...
if len(dataset) == 0:
    ...
    print('Accuracy of the network on the test dataset: {:.2f}%'.format(100 * correct / total))
